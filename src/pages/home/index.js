    ...
    <S.HomeElement>
      <S.HomeContainer>
        ...
        <S.HomeArticlesContainer>
          <S.HomeArticlesUl>
            {articlesHome.length
              ? articlesHome.map((item) => (
                <S.HomeArticlesli key={item.id}><ArticleCard item={item} size="small" handleClickBookmarks={() => saveBookmark(item, 'save', user.sub)} /></S.HomeArticlesli>)
              ): ''}
          </S.HomeArticlesUl>
        </S.HomeArticlesContainer>
        {showLoadMore && (
